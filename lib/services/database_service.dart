...
class DatabaseService {
  ...
  Future<void> insertEntry(FuelEntry entry) async {
    ...
    await db.insert('fuel_entries', entry.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<List<FuelEntry>> getAllEntries() async {
    final db = await database;
    final maps = await db.query('fuel_entries', orderBy: 'date DESC');
  ...
  Future<List<FuelEntry>> getEntriesOrderedByOdometer() async {
    final db = await database;
    final maps =
        await db.query('fuel_entries', orderBy: 'odometer_reading ASC');
    return maps.map((m) => FuelEntry.fromMap(m)).toList();
  }
  ...
  Future<Map<String, dynamic>> getStats() async {
    ...
    for (int i = 1; i < entries.length; i++) {
      final distance =
          entries[i].odometerReading - entries[i - 1].odometerReading;
    ...
    double avgMileage = mileages.isNotEmpty
        ...
        : 0.0;
    double bestMileage = mileages.isNotEmpty
        ? mileages.reduce((a, b) => a > b ? a : b)
        : 0.0;
    double worstMileage = mileages.isNotEmpty
        ? mileages.reduce((a, b) => a < b ? a : b)
        : 0.0;
